...
**Architecture:** MySQL remains the transactional source of truth and Elasticsearch is introduced as a read model behind a small Laravel service boundary. The implementation is split into environment/config, relational domain, and Elasticsearch operations/sync so each slice can be tested independently and then composed.
