...
- Feature folders may use nested `components/` folders, flat `hooks/` and `api/` folders, and `screens/` for feature-owned page composition.
...
- Use folder-local barrel files to expose stable feature entrypoints.
- Avoid large app-wide barrel files; keep the public API scoped to the feature or subfolder.

## Current Domain Mapping

- `features/jobs/` owns:
  - thin public exports from `index.ts`
  - `screens/search-screen.tsx` and `screens/detail-screen.tsx`
  - nested `components/search`, `components/detail`, and `components/shared`
  - flat feature-local `hooks/` and `api/`
...
- Importing another feature's private files instead of its barrel exports.

## Refactor Notes

- This structure preserves Inertia's `pages/**/*.tsx` route resolution.
- Existing route entry files stay in place as thin wrappers.
